 ...
 */
public class LorawanDataMessagePayload {
	 ...
	 * @param data the payload */
	 public LorawanDataMessagePayload(byte[] data) {
	...
	public LorawanDataMessagePayload(byte[] buf, int off, int len) {
	...
	public int getLength() {
		...
		if (encryptedPayload.length>0) len+=1+encryptedPayload.length;
		return len;
	}
	...
	public void decryptFramePayload(byte[] key) throws GeneralSecurityException {
		decryptedPayload=encrypt(key,encryptedPayload);
	}
	...
	public byte[] getFramePayload(byte[] key) throws GeneralSecurityException {
		return decryptedPayload=encrypt(key,encryptedPayload);
	}
